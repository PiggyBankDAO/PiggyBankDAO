...
import { StakingHelperContract, PBTokenContract, SpbTokenContract, StakingContract } from "../../abi";
...
export const changeApproval = createAsyncThunk("stake/changeApproval", async ({ token, provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const pbContract = new ethers.Contract(addresses.PB_ADDRESS, PBTokenContract, signer);
    const spbContract = new ethers.Contract(addresses.SPB_ADDRESS, SpbTokenContract, signer);
    ...
    try {
        ...
        if (token === "time") {
            approveTx = await pbContract.approve(addresses.STAKING_HELPER_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
        }

        if (token === "spb") {
            approveTx = await spbContract.approve(addresses.STAKING_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
        }
    ...
    const stakeAllowance = await pbContract.allowance(address, addresses.STAKING_HELPER_ADDRESS);
    const unstakeAllowance = await spbContract.allowance(address, addresses.STAKING_ADDRESS);

    return dispatch(
        fetchAccountSuccess({
            staking: {
                pbStake: Number(stakeAllowance),
                spbUnstake: Number(unstakeAllowance),
            },
        }),
    );
});
