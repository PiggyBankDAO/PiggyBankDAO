...
export class LPBond extends Bond {
    ...
    public getPBAmount(networkID: Networks, provider: StaticJsonRpcProvider) {
        return this.getReserves(networkID, provider, false);
    }

    public async getBalanceOf(networkID: Networks, pbContract: Contract) {
        const balance = await pbContract.balanceOf(super.getAddressForBond(networkID)) / Math.pow(10, 9);
        return balance;
    }

    public async getReverseBalanceOf(networkID: Networks, pbContract: Contract) {
        const lpReserveAddress = super.getAddressForReserve(networkID);
        const balance = await pbContract.balanceOf(lpReserveAddress) / Math.pow(10, 9);
        return balance;
    }
    ...
    private async getReserves(networkID: Networks, provider: StaticJsonRpcProvider, isToken: boolean): Promise<number> {
        ...
        const isTime = token1.toLowerCase() === addresses.PB_ADDRESS.toLowerCase();
